...
def get_buy_item(date_range):
	return OrderItem.objects.filter(Q(option='buy')|Q(option='eBuy'), 
	order__complete=True,
	order__orderTime__range=date_range)

def get_rent_item(date_range):
	return OrderItem.objects.filter(option='eRent', 
	order__complete=True,
	order__orderTime__range=date_range)
...
def get_all_item(date_range):
	...
	order__complete=True,
	order__orderTime__range=date_range).annotate(
	number=Case(When(option='eRent', then=1),
		default='quantity')
	)
...
def save_error_payment(request, form, form_temp, list_temp):
	...
	if request.method == "POST":
		if form.is_valid():
			...
			data['list'] = render_to_string(list_temp,{'error_payment_list': error_payment_list,})
		else:
			data['form_is_valid'] = False
