...
def dashboard(request):
    ...
    item_list = (
        get_buy_item(date_range)
        .values("book__ISBN", "book__name", "book__price")
        .annotate(book_count=Sum("quantity"))
    )
    top_item_list = item_list.order_by("-book_count")[:5]

    total_eRent = get_rent_item(date_range).count()
    total_buy = item_list.filter(option="buy").aggregate(total=Sum("book_count"))[
        "total"
    ]
    total_eBuy = item_list.filter(option="eBuy").aggregate(total=Sum("book_count"))[
        "total"
    ]
...
def error_transaction(request):
    error_payment_list = Payment.objects.exclude(status="finish").select_related(
        "customer"
    )
...
def author(request):
    if request.method == "GET":
        ...
        month_range = [
            start_day.replace(day=1).strftime("%Y-%m-%d"),
            next_month.replace(day=1).strftime("%Y-%m-%d"),
        ]

        top_author_day = (
            get_buy_item(date_range)
            .values("book__authors__id", "book__authors__name", "book__authors__email")
            .annotate(book_count=Sum("quantity"))
            .order_by("-book_count")[:5]
        )
        top_author_month = (
            get_buy_item(month_range)
            .values("book__authors__id", "book__authors__name", "book__authors__email")
            .annotate(book_count=Sum("quantity"))
            .order_by("-book_count")[:5]
        )
        ...
        data["list"] = render_to_string(
            "author/top_author_item.html", {"top_author": top_author}
        )
...
def storage(request):
    if request.method == "GET":
        ...
        date_range = [start_day.strftime("%Y-%m-%d"), end_day.strftime("%Y-%m-%d")]
        top_export = (
            Storage.objects.filter(export__exportTime__range=date_range)
            .annotate(export_num=Sum("export__quantity"))
            .order_by("-export_num")[:5]
        )

        storage = request.user.staff.storage
        item_list = (
            Inventory.objects.filter(storage=storage)
            .order_by("quantity")
            .select_related("book__book")
        )

        lack_item_list = (
            Inventory.objects.filter(storage=storage, quantity__lt=10)
            .order_by("quantity")
            .select_related("book__book")
        )
        ...
        data["form_is_valid"] = True
        data["list"] = render_to_string(
            "storage/top_storage_item.html", {"top_export": top_export}
        )
        return JsonResponse(data)
...
def order(request):
    staff = request.user.staff
    order_list = (
        Order.objects.filter(Q(staff=staff) | Q(staff__isnull=True), complete=True)
        .order_by("-orderTime")
        ...
        for item in item_list:
            try:
                if item.option == "buy":
                    ...
                    export_item = Export(
                        book=book, staff=staff, storage=storage, quantity=item.quantity
                    )
                    export_item.save()
            except:
                pass
...
def payment(request):
    if request.method == "GET":
        ...
        end_day = start_day + timedelta(days=1)
        date_range = [start_day.strftime("%Y-%m-%d"), end_day.strftime("%Y-%m-%d")]
        card_payment_list = Payment.objects.filter(
            method="credit", paymentTime__range=date_range
        ).select_related("customer")
        error_payment_list = (
            Payment.objects.filter(method="credit")
