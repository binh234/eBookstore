...
class ImportForm(BootstrapHelperForm, ModelForm):
	book = forms.ModelChoiceField(queryset=Traditional.objects.all().order_by("book__name"))
	class Meta:
		model = Import
...
class BookForm(BootstrapHelperForm, ModelForm):
	...
	authors = forms.ModelMultipleChoiceField(queryset=Author.objects.all().order_by("name"))
	...
	def __init__(self, *args, **kwargs):
		...
		if instance:
			self.fields['topics'].initial = ", ".join(x.name for x in instance.topic_set.all())
			self.fields['keywords'].initial = ", ".join(x.keyword for x in instance.keyword_set.all())
	...
	def save_extra(self, book):
		...
		list(map(lambda topic: Topic.objects.get_or_create(book=book, name=topic), topic_list))
		list(map(lambda keyword: Keyword.objects.get_or_create(book=book, keyword=keyword), keyword_list))
