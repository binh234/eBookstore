...
class RegisterForm(UserCreationForm):
    ...
    def clean_password2(self):
        ...
        if len(password2) < 8:
            errors.append(
                forms.ValidationError(
                    """Mật khẩu phải có ít nhất 8 kí tự""", code="short_password"
                )
            )
        if password2.isdigit() or password1.isdigit():
            errors.append(
                forms.ValidationError(
                    """Mật khẩu phải chứa ít nhật 1 kí tự chữ""",
        ...
        if password2 != password1:
            errors.append(
                forms.ValidationError(
                    """Mật khẩu không trùng khớp""", code="password_mismatch"
                )
            )
...
class CardForm(ModelForm):
    ...
    expireDate = forms.CharField(
        label="Ngày hết hạn", widget=forms.DateInput(attrs={"type": "date"})
    )
